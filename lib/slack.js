  ...
  return new Promise((resolve, reject) => {
    slack.channels.list({token}, (err, data)=> {
      if (err) reject (err);
        ...
        //console.log("Channel members: " , data.channels);
        var general;
        for (var i = 0; i < data.channels.length; i++){
          if (data.channels[i].is_general == true){
            console.log("General Channel", data.channels[i].id);
            general = JSON.stringify(data.channels[i].id);
          }
        }
        resolve (general);
      };
