...
var help_status = {status: false, index: 0}
...
exports.recap = (message) => {
  ...
  // if (help_status.status) {
  //   console.log("Currently helping");

  //   channel = result.channel.id;
  //   username = "recaptain";
  //   if (help_status.index == 1) {
  //     text = ("Hey " + name + ", HELP HELP HELP HELP HELP HELP HELP");
  //     help_status.status = false;
  //     help_status.index = 0;
  //   }
  //   attach =  [{"title": "For Example:", 
  //     "text": "To get a recap of the ", 
  //     "color": "#36a64f",
  //     "mrkdwn_in": [
  //       "text",
  //       "title"
...
exports.hi = (message, users_in_convo) => {
    let { channel, text, user, username, ts, general } = message;
      ...
      .then(getHandle(user)
        ...
        .then((result) => {
          if(result.channel.id == channel) {
            ...
            if (help_status.index == 0) {
              text = ("Hello " + name + "! My name is Recaptain. I'm a friendly bot that helps you summarize channels in slack. My job is to look at all the conversations from a channel and tell you what was important. Let's get started by typing\n```recap <#C0AS7C51D>```\nin this text box :point_down: to get a summary of the General Channel.");
              help_status.status = true;
              help_status.index += 1;
              users_in_convo[username] = 0;
            }
